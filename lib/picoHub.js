...
function requestTemp(sock) {
  var b = prepareMsg({ jsonrpc: "2.0", id: _msgId, method: "temp/room", params: { }});
...
function requestLight(sock) {
  var b = prepareMsg({ jsonrpc: "2.0", id: _msgId, method: "light/room", params: {  }});
  _requestLightId = _msgId;
  _msgId++;
...
function receiveMessage(clientId,msg) {
  ...
  if (msg.payload.hasOwnProperty("id")) {
    if (msg.payload.hasOwnProperty("result")) {
      ...
      } else if (msg.payload.id == _requestLightId) {
        var key = msg.from + "/light/room";
        updateDataCache(key,msg.payload.result);
        _requestLightId = 0;
        setTimeout(function() { requestLight(_clients[clientId]); }, 15000);
      }
