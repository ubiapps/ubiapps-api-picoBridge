(function() {
  ...
  function PicoBridgeImpl(cfg,rpcHandler) {
    ...
    picoHub.subscribe(cfg.serviceId, onData.bind(this));
  }

  var onService = function(device, type, instance) {
    var serviceParams = {
      deviceName: device,
      serviceType: type,
      serviceId: device + "/" + type + "/" + instance
    };
  ...
  var getCurrent = function(successCB, errorCB) {
    var cached = picoHub.getServiceValue(this._config.serviceId);
    successCB(cached);
  };

  var setCurrent = function(val, successCB, errorCB) {
    var cached = picoHub.setServiceValue(this._config.deviceName, this._config.serviceType, this._config.serviceId, val);
    successCB(cached);
  };
  ...
  PicoBridgeImpl.prototype.setCurrent = function(val, successCB, errorCB) {
    setCurrent.call(this,val, successCB,errorCB);
  };

  PicoBridgeImpl.loadServices = function(rpcHandler, register, unregister, serviceClass) {
    setTimeout(picoHub.startHub, 1000);
  };
  ...
  picoHub.subscribeService(onService);
